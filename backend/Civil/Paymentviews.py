...
class CreateTransaction(generics.CreateAPIView):
    permission_classes = (IsAuthenticated,)
    queryset = TransactionModelCivil.objects.all()
    ...
    def post(self, request):
        try:
            ...
            if order == '' or bank == '':
                ...
                if CompanyAccount.objects.filter(id=bank).exists() and OrderCivil.objects.filter(id=order).exists():
                    bankObj = CompanyAccount.objects.get(id=bank)
                    orderobj = OrderCivil.objects.get(id=order)
                    ...
                    if serializer.is_valid(raise_exception=True):
                        ...
                        if orderobj.payment_left > 0:
                            OrderCivil.objects.filter(id=orderobj.id).update(total_online_paid = total_online_pay, payment_left= orderobj.payment_left-1)
                        else:
                            OrderCivil.objects.filter(id=orderobj.id).update(total_online_paid = total_online_pay)
...
class GetTransaction(generics.ListAPIView):
    ...
    def get(self, request):
        isinstance =  TransactionModelCivil.objects.filter(user = request.user).all().order_by('-id')
