...
def upload_to_payment_doc(instance, filename):
    return 'image/Civil/payment/bank_img/{filename}'.format(filename=filename)

def upload_to_bar_code(instance, filename):
    return 'image/Civil/payment/bar_code/{filename}'.format(filename=filename)

def upload_to_Pay_Receipt(instance, filename):
    return 'image/Civil/payment/Receipt/{filename}'.format(filename=filename)
...
class TransactionModelCivil(models.Model):
    bank = models.ForeignKey(CompanyAccount,on_delete= models.SET_NULL, null=True)
    user = models.ForeignKey(User,on_delete= models.SET_NULL, null=True)
    order = models.ForeignKey(OrderCivil,on_delete= models.SET_NULL, null=True)
